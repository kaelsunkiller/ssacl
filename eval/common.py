...
from eval.utils import load_data, norm_heatmap, biovil_show
...
class Pipeline:
    ...
    def __init__(self, inference: Callable, save_fig=False, save_dir=None, **kwargs):
    ...
    def test(self, path_list: list, label_text: list, hmaps: list, gtmasks: list, boxes: list, category:list, **kwargs):
        ...
        for threshold in threshold_list:
            ...
            for i in tqdm(range(len(path_list[:]))):
                ...
                #     nan = np.isnan(h)
                #     h = norm_heatmap(h, nan, mode=0)
                #     name = label_text[i].replace("/", "_")
                #     cxr_idx = len(path_dict) - 1
                #     dis_idx = path_dict[path_list[i]]
                #     save_path = f"{self.save_dir}/image/heatmap/{dataset}/{cxr_idx}_{dis_idx}_{name}_node{j}.png"
                #     biovil_show(path_list[i], h, bboxes=None, save_path=save_path, title=None, resize=h.shape[0])
                # continue
                ...
                if kwargs["save_fig"] and threshold == threshold_list[0]:
                    ...
                    if boxes[i] is None:
                        ...
                        nanmask = None
                    biovil_show(path_list[i], heatmap, bboxes=None, save_path=save_path, title=None, resize=heatmap.shape[0])
                    # biovil_show3(path_list[i], bboxes=None, save_path=save_path, title=None, resize=heatmap.shape[0])
