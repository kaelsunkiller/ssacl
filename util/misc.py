...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, best=False, timestamp=''):
    output_dir = Path(os.path.join(args.output_dir, args.prefix, timestamp, "model"))

    
    epoch_name = str(epoch) if not best else 'best'
