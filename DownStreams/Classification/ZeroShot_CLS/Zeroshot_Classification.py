...
from .ZeroShot import model_ssacl
from .ZeroShot.utils.my_dataset import RSNAPneumonia, NIHChestXray
...
class BaseEvaluateEngine:
    ...
    def get_arguments() -> argparse:
        ...
        parser.add_argument("--output_file", type=str, 
                            default="zeroshot_cls_results/test_evaluation_result.json") 
        
        parser.add_argument("--bert_type", default="ssacl")
        ...
        parser.add_argument("--pretrained_path", type=str,
                            default="pretrained_weights/ssacl-best.pth")
        parser.add_argument("--dataset", default="NIH", 
                            choices=["NIH", "RSNA"])
        parser.add_argument("--dataset_list", default=["NIH", "RSNA"], type=str)
    ...
    def load_model(self, args) -> torch.nn.Module:
        ...
        """
        # step-1: init model
        model = model_ssacl.ssacl(
            bert_type=args.bert_type,
            normalize_feature=args.normalize_feature,
            multi_class=args.multi_class,
            num_classes=len(self.dataset.categories),
        )
        # model.bert_encoder.model.bert.pooler.dense = nn.Linear(model.bert_encoder.model.bert.config.hidden_size, 768)
    ...
    def load_dataset(self, args) -> torch.utils.data.Dataset:
        ...
        if args.dataset == "NIH":
            ...
            dataset = self.available_dataloaders["NIH"](
                root="/path/to/dataset/ChestXray8/",
                root_split="DownStreams/Datasplits/NIH_ChestX-ray",
                data_volume="100",
                split="test",
            ...
            dataset = self.available_dataloaders["RSNA"](
                root="/path/to/dataset/rsna/",
                root_split="DownStreams/Datasplits/RSNA_Pneumonia",
                data_volume="100",
                split="test",
                transform=transform_test,
            )
